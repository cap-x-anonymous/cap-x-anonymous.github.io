...
function injectDynamicContent() {
  ...
  // Equal contribution note — only show if any author has equalContrib
  const equalNote = document.getElementById("equal-note");
  if (equalNote && SITE_CONFIG.authors.some(a => a.equalContrib))
    equalNote.textContent = "* Equal contribution";
